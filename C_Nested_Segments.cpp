...
void solve()
{
    ...
    vector<int>first(n,-1),second(n);
    for(int i= 0;i<n;++i){
        if(first[a[i]]==-1)first[a[i]] = i,st.set(i,1);
        else second[a[i]] = i,st.set(i,-1);
    }

    vector<int>res(n);
    for(int i = 0;i<n;++i){
        if(first[a[i]]==i){
            int w = second[a[i]]-first[a[i]]-1;
            int c = (w-st.sol(first[a[i]]+1,second[a[i]]))/2;
            res[a[i]] = c;
            st.set(second[a[i]],1);
        }
    }
